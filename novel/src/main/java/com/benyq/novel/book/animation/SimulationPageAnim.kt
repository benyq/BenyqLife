...
class SimulationPageAnim(w: Int, h: Int, view: View, listener: OnPageChangeListener): HorizonPageAnim(w, h, view, listener){
    ...
    var mColorMatrixFilter: ColorMatrixColorFilter? = null
    ...
    override fun startAnim() {
        ...
        if (isCancel) {
            dx = if (mCornerX > 0 && mDirection.equals(Direction.NEXT)) {
    ...
    fun drawCurrentPageShadow(canvas: Canvas) {
        ...
        rotateDegrees = Math.toDegrees(
            Math.atan2(
                (mTouchX - mBezierControl1.x).toDouble(), (mBezierControl1.y - mTouchY).toDouble()
            )
        ).toFloat()
        ...
        val temp: Float
        temp = if (mBezierControl2.y < 0) mBezierControl2.y - mScreenHeight else mBezierControl2.y
    ...
    fun calcCornerXY(x: Float, y: Float) {
        ...
        mIsRTandLB = if (mCornerX == 0 && mCornerY == mScreenHeight
